...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for production_type in PRODUCTION_TYPES:
            ...
            url = f"https://digital.iservices.rte-france.com/open_api/generation_installed_capacities/v1/capacities_per_production_type?start_date=2015-01-01T00:00:00%2B01:00&end_date={year}-01-01T00:00:00%2B01:00"
            params = {
            ...
            for element in data:
                date = datetime.datetime.strptime(element["start_date"].split("T")[0], '%Y-%m-%d')
                production_type = element["type"]
                value = int(element['value']) * 1000  # we want capacities to be expressed in kW
                if not InstalledCapacity.objects.filter(country=france, date=date, production_type=production_type).exists():
